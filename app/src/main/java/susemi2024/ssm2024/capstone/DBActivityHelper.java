...
public class DBActivityHelper {
    ...
    public long insertColumn(String name, String cate, int lyear, int lmonth, int lday, int ayear, int amonth, int aday,
                             String company, String memo, String image,String expirationDate,int usedate, int isChecked) { //user table insert
        ...
        values.put(DBActivity.COL_USEDATE, usedate);
        values.put(DBActivity.COL_EXPIRATION_DATE,expirationDate);
        values.put(DBActivity.COL_IS_CHECKED,isChecked);
    ...
    public Cursor select(String[] colums,
                         ...
                         String groupBy,
                         String having,
                         String orderby) { //select 인자에 맞춘 질의문입니다. 조건에 맞춰 null을 사용하면 됩니다.
    ...
    public boolean updateColumn(int id, String name, String cate, int lyear, int lmonth, int lday, int ayear, int amonth, int aday
            , String company, String memo, String image, int usedate, String expirationDate,int isChecked) {
        ContentValues values = new ContentValues();
        values.put(DBActivity.COL_NAME, name);
        values.put(DBActivity.COL_CATE, cate);
        values.put(DBActivity.COL_LYEAR, lyear);
    ...
    public boolean updateCheckbox(int id, String expirationDate) {
        ContentValues values = new ContentValues();
        values.put(DBActivity.COL_EXPIRATION_DATE, expirationDate); // 변경된 부분
        return mDB.update(DBActivity._TABLENAME, values, "_id=" + id, null) > 0;
    }
}